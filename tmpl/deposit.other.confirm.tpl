                ...
                <div class="card">
                    ...
                    <div class="body">
                        <div class="table-responsive">
                            ...
                            <span style="color: #facd8a;padding: 8px;">{$description}</span><br><br>
                            ...
                            <table cellspacing=0 cellpadding=2 class="table table-hover m-b-0">
                                ...
                                <tr>
                                    <th style="text-align: left;color: #facd8a;">{$deposit.converted_fiat} Amount:</th>
                                    <td style="text-align: right;">{$deposit.amount|number_format:2}</td>
                                </tr>
                                {else}
                                <tr>
                                    <th style="text-align: left;color: #facd8a;">Amount:</th>
                            ...
                            <form name=spend method=post>
                                ...
                                <table cellspacing=0 cellpadding=2 border=0 class="table table-hover m-b-0">
                                    <tr>
                                        <td colspan=2><b style="color: #facd8a;">Required Information:</b></td>
                                    </tr>
                                    {section name=f loop=$fields}
                                    <tr>
                                        <td style="color: #facd8a;">{$fields[f].name}</td>
                                        <td><input type="text" name="fields[{$fields[f].id}]" value="" class="calculate-amount form-controlamount"></td>
                                    </tr>
                                    {/section}
                                </table>
                                {/if}

                                <br><input type=submit value="Save" class=deposit-process> &nbsp;
                            ...
                            <div class="alertnoti alert-warning">Sorry, the maximal deposit is {$max_deposit_format}.</div>
                            {/if}
                            ...
                            <div class="alertnoti alert-warning">The minimal spend amount for Bank Wire is {$currency_sign}{$min_amount}.</div>
                            {/if}
                            {if $less_than_min == 1}
                            <div class="alertnoti alert-warning">The minimal spend amount for '{$plan_name}' is {$currency_sign}{$min_amount}.</div>
                            {/if}
                            {if $zero_amount}
                            <div class="alertnoti alert-warning">You should enter amount grater than zero.</div>
                            {/if}
